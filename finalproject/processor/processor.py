...
def process_message(message):
    try:
        ...
        cursor.execute(
            """
            INSERT INTO songs (title, artist, rank, views, weeks_in_parade)
            VALUES (%s, %s, %s, %s, %s)
            """,
            (song_data['title'], ', '.join(song_data['artists']), song_data['rank'],
             song_data['views'], song_data['weeks_in_parade'])
        )
        conn.commit()
